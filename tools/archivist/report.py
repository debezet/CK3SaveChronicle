...
def render(c: Character, idx: CharacterIndex, memories: dict[int, Memory], current: Character | None = None) -> str:
    lines = []
    lines.append(f"# Raport Archiwisty — {c.name or c.id}")
    lines.append("")
    if current:
        lines.append(f"_Poprzedni władca względem aktualnej postaci: {idx.name(current.id)}._")
        lines.append("")
    lines.append("## Dane podstawowe")
    lines.append("")
    lines.append(f"- ID: `{c.id}`")
    lines.append(f"- Imię: {c.name or 'brak danych'}")
    lines.append(f"- Urodzony: {c.birth or 'brak danych'}")
    lines.append(f"- Zmarł: {c.death or 'brak danych'}")
    lines.append(f"- Przydomek: {c.nickname or 'brak'}")
    lines.append(f"- Przyczyna śmierci: {c.death_reason or 'brak danych'}")
    lines.append(f"- Kultura: {c.culture or 'brak danych'}")
    lines.append("")
    lines.append("## Rodzina")
    lines.append("")
    lines.append(f"- Ojciec: {idx.name(c.father) if c.father else 'brak danych'}")
    lines.append(f"- Matka: {idx.name(c.mother) if c.mother else 'brak danych'}")
    ...
    if c.memories:
        ...
        for mid in c.memories:
            ...
            lines.append(f"- `{mid}` — {mem.date or 'brak daty'} — `{mem.type or 'unknown'}`{extra}")
    else:
